...
async function getProviderDetail(apiEndpoint, owner) {
  ...
  const response = await axios.get('http://proxy-cors-005.herokuapp.com/'+ApiUrlService.providerDetail(apiEndpoint, owner));
...
async function getDataNodeProviders() {
  const response = await axios.get('http://proxy-cors-005.herokuapp.com/'+`${cloudmosApi}/providers`);
...
async function getNetworkCapacity() {
  const response = await axios.get('http://proxy-cors-005.herokuapp.com/'+ApiUrlService.networkCapacity());
...
async function getAuditors() {
  const response = await axios.get('http://proxy-cors-005.herokuapp.com/'+"https://raw.githubusercontent.com/maxmaxlabs/cloudmos-deploy/master/auditors.json");
