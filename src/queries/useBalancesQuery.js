...
async function getBalances(apiEndpoint, address) {
  ...
  const balancePromise = axios.get('https://proxy-cors-005.herokuapp.com/'+ApiUrlService.balance(apiEndpoint, address));
  const rewardsPromise = axios.get('https://proxy-cors-005.herokuapp.com/'+ApiUrlService.rewards(apiEndpoint, address));
  const redelegationsPromise = axios.get('https://proxy-cors-005.herokuapp.com/'+ApiUrlService.redelegations(apiEndpoint, address));
  const unbondingsPromise = axios.get('https://proxy-cors-005.herokuapp.com/'+ApiUrlService.unbonding(apiEndpoint, address));
  ...
  try {
    const delegationsResponse = await axios.get('https://proxy-cors-005.herokuapp.com/'+ApiUrlService.delegations(apiEndpoint, address));
