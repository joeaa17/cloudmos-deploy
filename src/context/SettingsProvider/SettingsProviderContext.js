...
export const SettingsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initiateSettings = async () => {
      ...
      const response = await axios.get('http://proxy-cors-005.herokuapp.com/'+currentNetwork.nodesUrl);
  ...
  const loadNodeStatus = async (nodeUrl) => {
    ...
    try {
      const response = await axios.get('http://proxy-cors-005.herokuapp.com/'+`${nodeUrl}/node_info`, { timeout: 10000 });
